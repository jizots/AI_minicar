...
def steer_ml(FRdis, RHdis, LHdis):
    X = np.array([[FRdis, RHdis, LHdis]])
    X_norm = (X - X_mean) / X_std
    return [-0.9,-0.6,-0.3,0.0,0.3,0.6,0.9][model.predict(X_norm)[0]]
    #return [0., 1., -1][model.predict(X_norm)[0]]
...
try:
    while True:
        ...
        steer = steer_ml(FRdis, RHdis, LHdis)
        ...
        #d = np.vstack([d,[time.time()-start_time, FRdis, RHdis, LHdis, RRHdis, RLHdis]])
        #距離を表示
        print('Fr:{0:.1f} , FrRH:{1:.1f} , FrLH:{2:.1f}, RrRH:{3:.1f} , RrLH:{4:.1f}'.format(FRdis,RHdis,LHdis))
